...
--  Remove dependency upon binary addins
--      Any external tools are annotated with **
--  Remove extman dependency
--  Maintain compatibility with SciTE 3.7.5 and Lua 5.1
--      For some reason my old system renders 3.7.5 better
...
--      Short function names for use in a Command Mode
...
--          hunspell
--          mac     Macros
--          mod     This file and most external tools
--          properties
--          tem     Templates
--          snp     Snippets
-- N.B. Collapse all folds for an overview

-- ***Compatibility*** for newer versions of SciTE
-- Set Lua search paths for binary addins (DLLs) - not used at present
package.cpath = props["SciteDefaultHome"].."\\mod\\?.dll"..";"..package.cpath

if not loadstring then                  -- Assume Scite 4.4.4 i.e. Lua 5.3
    -- Make loadstring
    loadstring = load
else                                    -- Assume Scite 3.7.5 i.e. Lua 5.1
    -- Make scite constants
    for i=1,5050 do
        if pcall(scite.ConstantName,i) then
            _G[scite.ConstantName(i)]=tonumber(i)
        end
    end
    -- TODO Make a better os.execute which does not flash
end

-- ***Extensions to Lua***              -- Overloading of the Lua Standard Libraries
-- I know it may be considered tasteless https://stackoverflow.com/a/2032066
...
-- I like the idea of a pipeline of string functions to allow macros and modal editing
...
-- ***Extensions to use only in SciTE***
-- Start a string processing pipe:
...
function word()                         -- Select and return current word
...
function line()                         -- Select and return current line
...
-- Midstream function to send text to scite output window
...
-- Text functions                       -- Use these if you are not in a pipe
...
-- "Super global variables" are held in props to survive Lua restart
...
-- Load a colour palette from a table
    ...
    -- Define the mapping of colours to specific elements
    local Colours = {
    ["colour.brace.highlight"]          ="fore:"..props["colour.green"],
    ["colour.brace.incomplete"]         ="fore:"..props["colour.yellow"],
    ["colour.code.comment.box"]         ="fore:"..props["colour.green"],
    ["colour.code.comment.line"]        ="fore:"..props["colour.green"],
    ["colour.code.comment.doc"]         ="fore:"..props["colour.foreground"],
    ["colour.code.comment.nested"]      ="fore:"..props["colour.foreground"],
    ["colour.text.comment"]             ="fore:"..props["colour.blue"],
    ["colour.other.comment"]            ="fore:"..props["colour.green"],
    ["colour.embedded.comment"]         ="fore:"..props["colour.foreground"],
    ["colour.embedded.js"]              ="fore:"..props["colour.foreground"],
    ["colour.notused"]                  ="back:"..props["colour.red"],
    ["colour.number"]                   ="fore:"..props["colour.cyan"],
    ["colour.keyword"]                  ="fore:"..props["colour.blue"],
    ["colour.string"]                   ="fore:"..props["colour.magenta"],
    ["colour.char"]                     ="fore:"..props["colour.magenta"],
    ["colour.operator"]                 ="fore:"..props["colour.foreground"],
    ["colour.preproc"]                  ="fore:"..props["colour.yellow"],
    ["colour.error"]                    ="fore:"..props["colour.yellow"],
    ["fold.back"]                       =props["colour.background"],
    }
...
-- This is required because the line selection mode includes char 1 on the next line
...
function scite.CloseStrips()            -- Close all strips TODO close builtin strips
...
function OnKey(KeyCode,Shift,Control,Alt) -- Hand rolled shortcut handler
    ...
    if scite.KeyActions[KeyIndex] then
        ...
        return 1 -- Stop native actions
    else
        return nil -- Use native actions
    end
end
...
scite.KeyActions = {                    -- Keyboard Shortcuts  - many
    ...
    ["[+Ctrl+Alt+Editor"] = function()  -- Toggle all headers TODO go up to visible line
        -- scite.FoldAll() -- Fails - not yet defined
    ...
    ["?+Ctrl+Alt+Editor"] = function()  -- Run Lua Macro Strip
        ...
        scite.StripSetList(1,string.sort(table.concat(io.dir(props["SciteDefaultHome"]..[[\mac\*.*]]),"\n")) or "")
...
--                     scite.SendEditor(SCI_GRABFOCUS)	-- Focus on editor
    ...
    ["?+Alt+Editor"] = function()       -- Create Lua Macro
        ...
        -- Insert template
        local file = assert(io.open(props["SciteDefaultHome"]..[[\tem\SciteMacro.lua]]))
        local contents = file:read'*a'
        file:flush()
        file:close()
        editor:AddText(contents)
        -- TODO Change working directory to props["SciteDefaultHome"]..[[\mac]]
...
--                     scite.SendEditor(SCI_GRABFOCUS)		-- Focus on editor
    ...
    ["#+Ctrl+Editor"] = function()      -- Open your shell
        os.execute([[start P:\MyPrograms\PROFILE\Tools\cmd\cmd_patched.bat ]]..props["FilePath"])
    end,
    ["#+Ctrl+Shift+Editor"] = function()-- Open Explorer
        os.execute("start "..[[%SystemRoot%\Explorer.exe ]]..props["FileDir"])
    end,
...
--         scite.MenuCommand(IDM_REPLACE)
    ...
    ["F+Ctrl+Alt+Editor"] = function()  -- TODO Find in files Strip
        scite.CloseStrips()             -- Close other Strips
        props["ActiveUserStrip"] = "FindinFiles"
    ...
    ["H+Ctrl+Alt+Editor"] = function()  -- Replace in files **external tool
        ...
        scite.MenuCommand(IDM_SAVE)
        os.execute("start "..props["SciteDefaultHome"].."\\mod\\gwFindReplace.exe /portable /searchpath:"..props["FileDir"].." /filemask:".."*.*".."  /searchfor:"..editor:GetSelText().." /size:-1 /s:yes /h:yes,", nil, false, false)
    end,
...
--                     scite.SendEditor(SCI_GRABFOCUS)		-- Focus on editor
    ...
    ["L+Ctrl+Editor"] = function()      -- Lookup definition TODO search outside SciteLua!
        local text = sel() or scite.CurrentWord()
        if not text then return end
        local t = string.split(text,".")
        local target = _G
        if type(t)=="table" then
            for i=1,#t do
                ...
                if type(target)=="function" then
                    trace(debug.which(target)..text.."\n")
                end
            end
        elseif type(_G[text])=="function" then
            trace(debug.which(_G[text])..text.."\n")
        end
    end,
    ["L+Ctrl+Alt+Editor"] = function()  -- Locations strip
        -- NB I had to disable Hewlitt Packard sysinfo.exe to reclaim Alt+Ctrl+L
        local LocationCache = scite.GetCache("LocationCache") or {}
        scite.CloseStrips()             -- Close other Strips
        props["ActiveUserStrip"] = "Location"
        scite.StripShow("'   Location '{}((OK))")
        -- Dsiplay list of locations
...
--                 scite.StripShow("")
    ...
    ["N+Ctrl+Alt+Editor"] = function()  -- Open New Editor
        os.execute("start "..props["SciteDefaultHome"]..[[\scite32.exe "-check.if.already.open=0"]])
    end,
...
--                     scite.SendEditor(SCI_GRABFOCUS)		-- Focus on editor
    ...
    ["S+Ctrl+Alt+Editor"] = function()  -- Snippets
        local SnippetCache = scite.GetCache("SnippetCache") or {}
        scite.CloseStrips()             -- Close other Strips
        props["ActiveUserStrip"] = "Snippet"
        scite.StripShow("'  Snippet '{}((OK))")
        scite.StripSetList(1,string.sort(table.concat(io.dir(props["SciteDefaultHome"]..[[\snp\*.*]]),"\n")) or "")
    ...
    ["T+Ctrl+Alt+Editor"] = function()  -- New File then Insert Template Strip
        ...
        scite.StripSetList(1,string.sort(table.concat(io.dir(props["SciteDefaultHome"]..[[\tem\*.*]]),"\n")) or "")
    ...
    ["W+Alt+Editor"]  = function()      -- List Recent Buffers
        scite.CloseStrips()             -- Close other Strips
...
-- Duplicates to be triggered in Strips
...
function scite.Backup()                 -- Make a backup file from the disc copy then saves
    ...
    if not io.exists(props["FileDir"].."\\zBackup") then
        ...
        scite.Message("Backup saved : "..props["FileDir"].."\\zBackup\\"..os.datestamp().."_"..props["FileName"].."."..props["FileExt"])
        -- Alt save file after?
        scite.MenuCommand(IDM_SAVE)
        return
    end
...
function scite.FoldTop()                -- Toggle Folds at Header and TODO goto top visible line
    scite.MenuCommand(IDM_TOGGLE_FOLDALL)
    scite.SendEditor(SCI_LINEUP)
    scite.SendEditor(SCI_SCROLLCARET)
end
function scite.FoldCurrent()            -- Toggle Fold this level and TODO goto top visible line
    scite.MenuCommand(IDM_EXPAND)
    scite.SendEditor(SCI_LINEUP)
    scite.SendEditor(SCI_SCROLLCARET)
end
function scite.FoldAll()                -- Toggle All levels and TODO goto top visible line
    scite.MenuCommand(IDM_TOGGLE_FOLDRECURSIVE)
    scite.SendEditor(SCI_LINEUP)
    scite.SendEditor(SCI_SCROLLCARET)
end
...
function scite.CheckSpelling()          -- Mark misspellings **external tool
    ...
    local UserDic = scite.LoadDic(props["SciteUserHome"].."\\user.dic") or {}
    local TypeDic = scite.LoadDic(props["SciteDefaultHome"].."\\dic\\"..props["FileExt"]..".dic") or {}
...
function scite.AddToUserDict()          -- Store acceptable words for user
    ...
    if editor:IndicatorValueAt(scite.SetIndicator("spelling"),editor.CurrentPos-1) ~= 0 then
        local fp = io.open(props["SciteUserHome"].."\\user.dic","a+")
...
function scite.AddToTypeDict()          -- Store acceptable words for file type
    -- This will only work for misspellings
    if editor:IndicatorValueAt(scite.SetIndicator("spelling"),editor.CurrentPos-1) ~= 0 then
        local fp = io.open(props["SciteDefaultHome"].."\\dic\\"..props["FileExt"]..".dic","a+")
        fp:write(scite.CurrentWord().."\n")
        fp:flush()
        fp:close()
    end
...
function scite.PastePlainTxt()          -- Paste plain text ** external tool
    os.execute(props["SciteDefaultHome"].."..\\mod\\GetPlainText.exe","",false,true)
    scite.MenuCommand(IDM_PASTE)
end
function scite.HelpIndex()              -- Search local index for selection ** external tool
...
scite.DefineTool("Spelling","dostring scite.CheckSpelling()") -- TODO Failure to launch!
